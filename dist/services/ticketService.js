import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, Colors, EmbedBuilder, PermissionsBitField, StringSelectMenuBuilder, } from "discord.js";
import { env } from "../config/env.js";
import { CustomIds } from "../constants/customIds.js";
function getManagementMention() {
    return env.managementRoleId ? `<@&${env.managementRoleId}>` : "Management";
}
function buildTicketButtons() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(CustomIds.ticketPurchaseButton).setLabel("🛒 Purchase / Satın Alma").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(CustomIds.ticketSupportButton).setLabel("✉️ Support / Teknik Destek").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(CustomIds.ticketInquiryButton).setLabel("ℹ️ Product Inquiry / Ürün Sorgula").setStyle(ButtonStyle.Secondary));
}
function buildCloseButton() {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(CustomIds.ticketCloseButton).setLabel("❌ Close Ticket / Ticketi Kapat").setStyle(ButtonStyle.Danger));
}
...
export function createFeedbackPanelComponents() {
    return [
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(CustomIds.feedbackOpenModalButton).setLabel("✍️ Submit Feedback / Geri Bildirim Gönder").setStyle(ButtonStyle.Primary)),
    ];
}
export function createTicketPanelComponents() {
    return [buildTicketButtons()];
}
export function createTicketPanelEmbedForChannel() {
    return buildTicketPanelEmbed();
}
export function createFeedbackPanelEmbedForChannel() {
    return buildFeedbackPanelEmbed();
}
export function createPurchaseSelectMenu(products) {
...
export function buildPurchaseSelectionPayload(products) {
    return {
        embeds: [
            new EmbedBuilder()
                .setColor(Colors.DarkRed)
...
function buildTicketEmbed(title, bodyLines, product, licenseKey) {
    const embed = new EmbedBuilder().setColor(Colors.DarkRed).setTitle(title).setDescription(bodyLines.join("\n")).setTimestamp();
    if (product) {
        embed.addFields({ name: "Selected Product / Seçilen Ürün", value: product.name, inline: false }, { name: "Product Price / Ürün Fiyatı", value: product.price, inline: false });
    }
    if (licenseKey) {
...
async function createPrivateTicketChannel(interaction, channelName, embed) {
...
export async function handleFeedbackSubmission(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "User", value: `${interaction.user} (${interaction.user.tag})`, inline: false }, { name: "License Key / Lisans Anahtarı", value: licenseKey, inline: false }, { name: "Feedback / Geri Bildirim", value: feedbackText.slice(0, 1024), inline: false })
        .setTimestamp();
    await guildChannel.send({ embeds: [embed] });
    await interaction.editReply("Teşekkürler! Geri bildiriminiz başarıyla iletildi.");
}
async function logTicketCreation(interaction, embed) {
...
export async function createPurchaseTicket(interaction, product) {
    if (!interaction.inCachedGuild()) {
        throw new Error("Bu işlem yalnızca önbelleğe alınmış sunucularda kullanılabilir.");
    }
    const channel = await createPrivateTicketChannel(interaction, `purchase-${interaction.user.id}`, buildTicketEmbed("Ticket System (Purchase Request / Satın Alma)", [
    ...
    const ticketLogEmbed = new EmbedBuilder()
        ...
        .addFields({ name: "User", value: `${interaction.user} (${interaction.user.tag})`, inline: false }, { name: "Channel", value: `${channel}`, inline: false }, { name: "Selected Product / Seçilen Ürün", value: product.name, inline: false }, { name: "Product Price / Ürün Fiyatı", value: product.price, inline: false })
        .setTimestamp();
    await logTicketCreation(interaction, ticketLogEmbed);
}
export async function createLicenseTicket(interaction, ticketType, sourceCustomId) {
    ...
    const channel = await createPrivateTicketChannel(interaction, `${channelPrefix}-${interaction.user.id}`, buildTicketEmbed(`Ticket System (${ticketType} / Lisans Doğrulama)`, [
    ...
    const ticketLogEmbed = new EmbedBuilder()
        ...
        .addFields({ name: "User", value: `${interaction.user} (${interaction.user.tag})`, inline: false }, { name: "Channel", value: `${channel}`, inline: false }, { name: "License Key / Lisans Anahtarı", value: licenseKey, inline: false })
        .setTimestamp();
    await logTicketCreation(interaction, ticketLogEmbed);
}
...
export function buildProductDeleteMenu(products) {
    const menu = new StringSelectMenuBuilder()
...
export function buildProductDeleteNotice(productsCount) {
    return new EmbedBuilder()
        .setColor(Colors.Orange)
        .setTitle("Ürün Silme Paneli")
        .setDescription(productsCount === 0
